...
When new content is pushed to the main branch, pre-releases are made. To make a non-pre-, latest release, run the `Make Latest Release` workflow on main. This will then push the docker images as latest as well.

On the branch called `canary` pre-releases are also made. These cannot be made into latest or non-pre-releases automatically. It is not adviced to make that branch have a latest release. To make a pre-release on `canary` a non-pre release, edit the release manually.
